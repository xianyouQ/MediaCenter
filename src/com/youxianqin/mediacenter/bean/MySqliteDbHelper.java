...
public class MySqliteDbHelper extends SQLiteOpenHelper{
	...
	public void onCreate(SQLiteDatabase arg0) {
		...
		arg0.execSQL(constant.CREATE_TABLE_VIDEO_SQL);
	}
	...
	public void insertToVideo(Video Videohistory)
	{
		SQLiteDatabase db =getWritableDatabase();
		Log.e("Videohistory db",Videohistory.GetLastPos()+" "+Videohistory.GetLastTime()
				  );
		long DeleteTime=System.currentTimeMillis()-604800000;
		db.delete(constant.TABLE_VIDEO, constant.TABLE_LAST_PLAY_TIME_COLUMN+"<?", new String[]{DeleteTime+""});
		db.delete(constant.TABLE_VIDEO, constant.TABLE_VIDEO_URL_COLUMN+"=?", new String[]{Videohistory.GetVideoUrl()});
		ContentValues cv = new ContentValues();
		...
		cv.put(constant.TABLE_VIDEO_ID_COLUMN, Videohistory.GetVideoId());
		cv.put(constant.TABLE_VIDEOPARENT_NAME_COLUMN, Videohistory.GetVideoParent());
		cv.put(constant.TABLE_VIDEO_DURATION,Videohistory.GetDuration());
		db.insert(constant.TABLE_VIDEO, null, cv);
		db.close();
	}
	
	public ArrayList<Video> queryVideo(String VideoParent)
	{
		ArrayList<Video> VideoHistoryList = new ArrayList<Video>();
		SQLiteDatabase db =getWritableDatabase();
		Cursor cs=db.query(constant.TABLE_VIDEO, null, constant.TABLE_VIDEOPARENT_NAME_COLUMN+"=?", new String[]{VideoParent}, null, null, null);
		for(cs.moveToFirst();!cs.isAfterLast();cs.moveToNext())
		{
			Video history = new Video();
			...
			history.SetVideoId(cs.getInt(cs.getColumnIndex(constant.TABLE_VIDEO_ID_COLUMN)));
			history.SetVideoParent(cs.getString(cs.getColumnIndex(constant.TABLE_VIDEOPARENT_NAME_COLUMN)));
			history.SetDuration(cs.getLong(cs.getColumnIndex(constant.TABLE_VIDEO_DURATION)));
